...
u8 lwip_comm_mem_malloc(void)
{
	...
	memp_memory=mymalloc(SRAMIN,mempsize);	//为memp_memory申请内存   19k/8k
	ramheapsize=LWIP_MEM_ALIGN_SIZE(MEM_SIZE)+2*LWIP_MEM_ALIGN_SIZE(4*3)+MEM_ALIGNMENT;//得到ram heap大小
	ram_heap=mymalloc(SRAMIN,ramheapsize);	//为ram_heap申请内存      16k/5k
	TCPIP_THREAD_TASK_STK=mymalloc(SRAMIN,TCPIP_THREAD_STACKSIZE*4);//给内核任务申请堆栈       4k
	LWIP_DHCP_TASK_STK=mymalloc(SRAMIN,LWIP_DHCP_STK_SIZE*4);		//给dhcp任务堆栈申请内存空间   1k
	if(!memp_memory||!ram_heap||!TCPIP_THREAD_TASK_STK||!LWIP_DHCP_TASK_STK)//有申请失败的
	{
		ETH_lwip_comm_Mem_Free();
...
//释放ETH 底层驱动申请的内存和lwip内核部分,内存释放
void ETH_lwip_comm_Mem_Free(void)
{
	ETH_Mem_Free();	
	lwip_comm_mem_free();
...
//      2,LAN8720初始化失败
//      3,网卡添加失败.
//注意：根据自己实际接收的数据大小可相应的修改接收最大数据包大小及个数来减少对ram内存的消耗
//     初始 ETH_Mem_Malloc（）测试时用了16k大小ram，lwip_comm_mem_malloc()测试时用了40k大小ram,\
//  修改后ETH_Mem_Malloc（）测试时用了16k大小ram，lwip_comm_mem_malloc()测试时用了18k大小ram,\
//   lwip内存修改可在lwipopts.h文件中修改
u8 lwip_comm_init(void)
{
	...
	if(ETH_Mem_Malloc())return 1;		//内存申请失败       16K
	if(lwip_comm_mem_malloc())return 1;	//内存申请失败   40k/18k
	if(DP83848_Init())
	{
		ETH_lwip_comm_Mem_Free();
		return 2;			//初始化DP83848失败 
	}
