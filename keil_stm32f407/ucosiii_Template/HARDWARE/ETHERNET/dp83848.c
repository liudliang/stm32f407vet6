...
__IO uint32_t  EthStatus = 0;       //以太网连接状态
...
u8 DP83848_GPIO_Config(void)
{
	...
	/*网络引脚设置 RMII接口
	  ...
	  ETH_RMII_TXD0 --------------------> PB12//PG13
	  ETH_RMII_TXD1 --------------------> PB13//PG14
//	  ETH_RESET-------------------------> PD3
	 */
...
{
	u8 speed;
	uint16_t speed1,mode1;
	speed=((ETH_ReadPHYRegister(DP83848_PHY_ADDRESS,PHY_SR)&0x1C)>>2); //从LAN8720的31号寄存器中读取网络速度和双工模式
	
	speed1 = ETH_ReadPHYRegister(DP83848_PHY_ADDRESS,PHY_SPEED_STATUS);	
	mode1 = ETH_ReadPHYRegister(DP83848_PHY_ADDRESS,PHY_DUPLEX_STATUS);	
...
{
	return (ETH_ReadPHYRegister(DP83848_PHY_ADDRESS, PHY_SR) & 1);
}
