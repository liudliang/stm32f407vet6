...
//     初始 ETH_Mem_Malloc（）测试时用了16k大小ram，lwip_comm_mem_malloc()测试时用了30k大小ram,\
//  修改后ETH_Mem_Malloc（）测试时用了16k大小ram，lwip_comm_mem_malloc()测试时用了14k大小ram,\
...
u8 lwip_comm_init(void)
{
	...
	if(DP83848_Init())return 2;			//初始化DP83848失败 
	lwip_init();						//初始化LWIP内核
...
/*LWIP 协议栈要求的是每 250ms 处理一次 TCP 定时器，每 5S 处理
一次 ARP 定时器，每 500ms 处理一次 DHCP 精细处理定时器，每 60s 执行一次 DHCP 的粗糙
处理*/
//LWIP轮询任务
