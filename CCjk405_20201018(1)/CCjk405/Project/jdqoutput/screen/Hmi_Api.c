...
uint8  Hmi_PackFrameBegin(uint8 *buf,uint8 cmd,uint16 addr)
{
	...
	if( CMD_WR_REG == cmd  || CMD_RD_REG == cmd)
	{
...
void Hmi_RequestPicNo(void)
{
	 uint8 len;
	 uint8 buf[32] = {0};
	 len = Hmi_PackQueryReg(buf,DGUS_PICNO_REG,CMD_RD_REG,2);
   Screen_UartWrite(buf,len);
}
...
uint8 Hmi_GetCurrPicNo(void)
{
   ...
   do
    ...
    if( (len < 9) && (buf[3] == 0x81) && (buf[4] == 0x03 ) ) {
			return 0xff;
		}
...
/*设置显示屏时间函数*/
/* 0x5A A5 0A 80 1F 5A 12 08 08 04 12 00 01 */
