...
extern OS_EVENT* SemEthnetSend;
extern OS_EVENT* SemEthnetRcv;	

extern uint32_t ETH_HandleTxPkt(uint8_t *ppkt, uint16_t FrameLength);	 
extern uint32_t ETH_HandleRxPkt(uint8_t *ppkt);

extern uint8 sysEthnetHwInit(void);
extern uint8 sysEthnetHwReset(void);
extern uint8 sysEthnetLink(void);	
